...
sub undo {
	...
	if($action->move_type eq "Day/Time") {
		...
		if($type eq 'undo') {
			...
			my $redo = Undo->new( $block->id, $block->start, $block->day, $action->origin_obj, $action->move_type);
			$self->add_redo($redo);
			...
			my $undo = Undo->new ( $block->id, $block->start, $block->day, $action->origin_obj, $action->move_type);
			$self->add_undo($undo);
	...
	if($type eq 'undo') {
		...
		my $redo = Undo->new( $action->block_id, $block->start, $block->day, $action->new_obj, $action->move_type, $action->origin_obj);
		$self->add_redo($redo);
		$self->remove_last_undo;
		$View::Undo_left = scalar $self->undoes ." undoes left";
		$View::Redo_left = scalar $self->redoes ." redoes left";
		...
		my $undo = Undo->new ( $action->block_id, $block->start, $block->day, $action->new_obj, $action->move_type, $action->origin_obj);
		$self->add_undo($undo);
		$self->remove_last_redo;
		$View::Undo_left = scalar $self->undoes ." undoes left";
		$View::Redo_left = scalar $self->redoes ." redoes left";
...
# get all teachers/labs/streams and sort in alphabetical order depending on type
	if ( $type eq "teacher" ) {
