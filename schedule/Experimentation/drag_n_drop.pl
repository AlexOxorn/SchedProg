...
# flat relief over the source, and a sunken relief over the destination.
    my $tl1     = $mw->Toplevel(-title=>'Drag From');
    my $tl2     = $mw->Toplevel(-title=>'Drag To');

my $c_src = $tl1->Canvas(qw/-background yellow/)->pack;
...
my $c_dest = $tl2->Canvas(qw/-background cyan/)->pack;
$c_dest->DropSite(
    -droptypes   => [qw/Local/],
...
sub move_bbox {
    my ($item_type, $c_src, $c_dest, $sel, undef,undef,$dest_x, $dest_y) = @_;
